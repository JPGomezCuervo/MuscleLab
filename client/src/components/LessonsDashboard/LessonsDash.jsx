...
import {
  selectAllLessonsDashboard,
  fetchAllLessonsDashboard,
  clearLesson,
            ...
            allLessons
              .filter(
                (el) =>
                  ...
                  el.types.some((type) => selectedTypes.includes(type))
              )
              .map((el) => (
                <div key={el.id}>
                  <div className={style.detalle}>
                    ...
                    <div className={style.contenido}>
                      <h2>
                        Nombre: <span>{el.name}</span>
                      </h2>
                      <h2>
                        status:{" "}
                        <span>{el.isAvailable ? "Activa" : "Inactiva"}</span>{" "}
                      </h2>
                      <h2>
                        Descripcion: <span>{el.shortDescription}</span>{" "}
                      </h2>
                      <h2>
                        Tipo de ejercicio: <span>{el.types?.join(", ")}</span>{" "}
                      </h2>
                    </div>
